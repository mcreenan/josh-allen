...
#[test]
fn exec_run_has_the_exact_lazy_signature_and_effect() {
    let source = r#"
export async fn main() returns Void effects [exec.run] {
  let result = await exec.run(
    ["printf", "%s", "hello; touch /tmp/not-a-shell"],
    Some(b"input")
  );
  ()
}
"#;
    let compilation = compile_source(source).expect("exec.run compiles");
    verify(compilation.module.clone()).expect("exec.run module verifies");
    let expected = ValueType::Future(Box::new(ValueType::Result(
    ...
    assert!(
        compilation
            .module
            .effect_sets
            .contains(&vec!["exec.run".to_owned()])
    );
        ...
        .find(|function| {
            function.code.iter().any(|instruction| {
                matches!(
                    instruction,
                    Instruction::EffectCall {
                        operation: EffectOperation::ExecRun,
                        ..
                    }
                )
            })
        })
        .expect("exec.run operation must be lowered");
    let destination = function
        ...
        .unwrap();
    assert_eq!(function.registers[usize::from(destination)], expected);

    let missing_effect = r#"
export async fn main() returns Void {
  let result = await exec.run(["printf"], None);
  ()
}
"#;
    let diagnostics = compile_source(missing_effect).expect_err("exec.run effect is required");
    assert_eq!(diagnostics[0].code, "E2403");
    assert_eq!(
        diagnostics[0].message,
        "function 'main' requires undeclared effects [exec.run]"
    );
}
...
"#;
    ...
    let operations = compilation
        ...
        .filter_map(|(function, instruction)| match instruction {
            Instruction::EffectCall { operation, .. } => Some((function, *operation, instruction)),
            _ => None,
        })
        .collect::<Vec<_>>();
    for operation in [
...
"#;
    let diagnostics =
        compile_source(invalid_grant_result).expect_err("permission result shape must be exact");
    assert_eq!(diagnostics[0].code, "E3007");
...
#[test]
fn collection_stats_zip_fold_and_immutable_map_updates_lower_to_typed_bytecode() {
    let source = r#"
export fn main() returns (List<(Int, String)>, Option<Int>, Option<Int>, Float, Int, List<String>) {
  let source = map { "b": 2, "a": 1 };
  let inserted = map.insert(source, "a", 3);
  let removed = map.remove(inserted.values, "b");
  (
    zip([1, 2], ["a", "b"]),
    list.min([3, 2, 2]),
    list.sum([1, 2, 3]),
    list.sum([1.0, 2.0]),
    list.fold([1, 2, 3], 0, fn(acc: Int, item: Int) returns Int { acc + item }),
    map.keys(removed.values)
  )
}
"#;
    let compilation = compile_source(source).expect("collection additions compile");
    verify(compilation.module.clone()).expect("collection additions verify");
    let verified = verify(compilation.module).expect("collection additions verify again");
    assert_eq!(
        allen_vm::execute(&verified)
            .expect("collection additions execute")
            .to_string(),
        "([(1, \"a\"), (2, \"b\")], Some(2), Some(6), 3.0, 6, [\"a\"])"
    );
}
...
fn every_production_compile_route_parses_each_source_once() {
    ...
    let (compilation, count) = syntax_lowering::count_parse_invocations(|| compile_source(loose));
...
fn hir_and_mir_have_resolved_ids_and_nested_effects() {
    fn inspect(
        ...
        match &expression.kind {
            ...
            | HirExprKind::TemplateRender { arguments, .. }
            | HirExprKind::StringOperation { arguments, .. }
            | HirExprKind::StandardOperation { arguments, .. }
            ...
            | HirExprKind::CollectionOperation { arguments, .. } => {
            ...
            HirExprKind::ListWithSpread(items) => {
                for item in items {
                    let value = match item {
                        HirListItem::Element(value) | HirListItem::Spread(value) => value,
                    };
                    inspect(
                        value,
                        type_count,
            ...
            HirExprKind::MapWithSpread(items) => {
                for item in items {
                    match item {
                        HirMapItem::Entry { key, value } => {
                            inspect(
                                key,
                                type_count,
                                effect_count,
            ...
            HirExprKind::ListFold {
                ...
                for child in [values.as_ref(), initial.as_ref(), callback.as_ref()] {
                    inspect(
                        child,
                        type_count,
                        effect_count,
            ...
            HirExprKind::For { source, body, .. } => {
                let HirForSource::Iterable(value) = source;
                inspect(
                    value,
                    type_count,
                    effect_count,
            ...
            HirExprKind::Slice { collection, range } => {
                for child in [collection.as_ref(), range.as_ref()] {
                    inspect(
                        child,
                        type_count,
                        effect_count,
            ...
            HirExprKind::SequenceFromList(value) | HirExprKind::SequenceToList(value) => inspect(
                value,
                type_count,
                effect_count,
                span_count,
            ...
            HirExprKind::SequenceMap { sequence, callback }
            | HirExprKind::SequenceFilter { sequence, callback }
            | HirExprKind::SequenceFind { sequence, callback }
            | HirExprKind::SequenceAny { sequence, callback }
            | HirExprKind::SequenceAll { sequence, callback } => {
                for child in [sequence.as_ref(), callback.as_ref()] {
                    inspect(
                        child,
                        type_count,
                        effect_count,
            ...
            HirExprKind::SequenceTake { sequence, count } => {
                for child in [sequence.as_ref(), count.as_ref()] {
                    inspect(
                        child,
                        type_count,
                        effect_count,
            ...
            HirExprKind::SequenceFold {
                ...
                for child in [sequence.as_ref(), initial.as_ref(), callback.as_ref()] {
                    inspect(
                        child,
                        type_count,
                        effect_count,
...
"#
    ...
    let compilation = compile_package_bundle_with_prepared_tools(&bundle, &mut prepared).unwrap();
    ...
    let input_contract_digest = allen_bytecode::compute_entry_contract_digest(
        &prepared.schemas[input_schema as usize],
        &[],
        &[],
    );
    let output_contract_digest = allen_bytecode::compute_entry_contract_digest(
        &prepared.schemas[output_schema as usize],
        &[],
        &[],
    );
    let input_record_provenance = allen_bytecode::compute_entry_record_provenance(
        &prepared.schemas[input_schema as usize],
    ...
    .unwrap();
    let output_record_provenance = allen_bytecode::compute_entry_record_provenance(
        &prepared.schemas[output_schema as usize],
        &compilation.module.enum_types,
        &[],
        &[],
    ...
    allen_bytecode::decode_and_verify(&bytes, &allen_bytecode::DecodeLimits::default()).unwrap();
}
...
fn closed_errors_learnxinyminutes_catalog_compiles_and_runs() {
    ...
    let bundle = PackageSourceBundle {
        ...
        sources: BTreeMap::from([
            ...
            (
                support.to_owned(),
                include_str!("../../../../examples/functions-and-effects/support.allen").to_owned(),
            ),
        ]),
        import_targets: BTreeMap::from([(
            (
...
export fn main() returns (Float, Bytes, String, Int, Void) {
  (12_5.0e-2, b"\"\\\n\r\t\0\b\f\x41", "\"\\\n\r\t\0\b\f", -9_223_372_036_854_775_808, ())
}
"#;
...
#[test]
#[allow(clippy::too_many_lines)]
fn numeric_literal_separators_lower_to_the_same_constants() {
    let source = r#"
export fn main() returns (Int, Int, Int, Int, Float, Float, Float) {
  (
    485_273,
    9_223_372_036_854_775_807,
    -9_223_372_036_854_775_808,
    -0_9223372036854775808,
    12_345.67_89e+1_0,
    1.0e9_9,
    1.0e-9_9
  )
...
"#;
    let compilation = compile_source(source).expect("separated literals compile");
    assert!(
        compilation
            .module
            .constants
            .contains(&Constant::Int(485_273))
    );
    assert!(
        compilation
            .module
            .constants
            .contains(&Constant::Int(i64::MAX))
    );
    assert!(
        compilation
            .module
    ...
    assert!(
        compilation
            .module
            .constants
            .contains(&Constant::Float(canonical_float_bits(
                123_456_789_000_000_f64.to_bits()
            )))
    );
    assert!(
        compilation
            .module
            .constants
            .contains(&Constant::Float(canonical_float_bits(1e99_f64.to_bits())))
    );
    assert!(
        compilation
            .module
            .constants
            .contains(&Constant::Float(canonical_float_bits(1e-99_f64.to_bits())))
    );
    verify(compilation.module).expect("separated literal module verifies");

    for (literal, error_offset) in [
        ("_1", 0),
        ("-_1", 1),
        ("1_", 0),
        ("-1_", 1),
        ...
        let source = format!("export fn main() returns Int {{ {literal} }}");
        let diagnostics = compile_source(&source).expect_err("invalid separator accepted");
        let start = source.find(literal).expect("literal is in source") + error_offset;
        assert_eq!(diagnostics.len(), 1, "{literal}: {diagnostics:?}");
        assert_eq!(diagnostics[0].code, "E0004", "{literal}");
        assert_eq!(
            diagnostics[0].message, "malformed number (S0003)",
            "{literal}"
        );
        assert_eq!(
            diagnostics[0].span,
            Span {
                start,
                end: source.find(literal).expect("literal is in source") + literal.len(),
            },
            "{literal}"
        );
        assert_eq!(
            &source[diagnostics[0].span.start..diagnostics[0].span.end],
            &literal[error_offset..]
        );
    }

    let source = "export fn main() returns Int { -0_9223372036854775809 }";
    let diagnostics = compile_source(source).expect_err("out-of-range magnitude accepted");
    assert_eq!(diagnostics.len(), 1);
    assert_eq!(diagnostics[0].code, "E3005");
    assert_eq!(
        diagnostics[0].message,
        "integer literal exceeds Int range (S0101)"
    );

    let source = "export fn main() returns Float { 1.0e9_999 }";
    let diagnostics = compile_source(source).expect_err("infinite Float literal accepted");
    let start = source.find("1.0e9_999").unwrap();
    assert_eq!(diagnostics.len(), 1);
    assert_eq!(diagnostics[0].code, "E0003");
    assert_eq!(diagnostics[0].message, "invalid Float literal");
    assert_eq!(
        diagnostics[0].span,
        Span {
            start,
            end: start + "1.0e9_999".len(),
        }
    );
}
...
"#;
    let compilation = compile_source(source).expect("declared callback effect unions are interned");
    let conditional_effects = vec!["fs.read".to_owned(), "fs.write".to_owned()];
...
"#
        ...
        assert!(matches!(diagnostics[0].code, "E3005" | "E3011"));
        assert_eq!(
            diagnostics[0].message,
...
"#;
    for flag in [true, false] {
        ...
        let verified =
            verify(compilation.module).expect("conditional callback collection bytecode verifies");
        if flag {
...
fn loops_literal_true_while_controls_fallthrough_exactly() {
    ...
    let compilation = compile_source(terminating).expect("literal true loop terminates control");
    let main = compilation
        .module
        .functions
        .iter()
...
"#;
    let compilation = compile_source(nonliteral).expect("nonliteral while may execute zero times");
    assert_eq!(
        allen_vm::execute(&verify(compilation.module).unwrap()),
        Ok(allen_vm::Value::Int(2))
    );
...
"#;
    let diagnostics =
        compile_source(invalid).expect_err("available SubAgent cannot cross an in-scope continue");
    assert_eq!(diagnostics[0].code, "E3011");
    assert!(diagnostics[0].message.contains("reachable continue"));
}
...
"#;
    let compilation =
        compile_source(source).expect("record match bindings retain their callback effect shapes");
...
"#;
    ...
    let verified =
        verify(compilation.module).expect("safe outer SubAgent call and Move provenance verifies");
...
"#;
    let diagnostics =
        compile_source(capture).expect_err("an actual SubAgent closure capture remains invalid");
    assert_eq!(diagnostics.len(), 1, "{diagnostics:?}");
    assert_eq!(diagnostics[0].code, "E3011");
    assert_eq!(
        diagnostics[0].message,
...
"#;
    ...
    let header =
        mir_goto_target(&unconditional.blocks[0]).expect("function entry reaches the loop header");
...
fn operators_remainder_type_errors_are_single_and_source_located() {
    ...
    let location = compilation
        ...
        .iter()
        .find(|location| {
            location.function as usize == function && location.instruction as usize == instruction
        })
...
"#;
    let diagnostics =
        compile_source(source).expect_err("a skipped branch cannot abandon a must-consume task");
    assert_eq!(diagnostics.len(), 1, "{diagnostics:?}");
    assert_eq!(diagnostics[0].code, "E3011");
...
#[test]
fn clockless_parse_format_and_replace_builtins_lower_to_verified_bytecode() {
    let source = r#"
export fn main() returns (
  ...
  (
    ...
    float.format(2.5, 0),
    time.format_utc(0),
    time.parse_utc("1970-01-01T00:00:00Z"),
    time.bucket(-1, 60),
    string.replace("éé", "é", "x")
  )
}
"#;
    let compilation = compile_source(source).expect("pure standard operations compile");
    let main = compilation
        .module
        .functions
        .iter()
    ...
    let operations = main
        ...
        .iter()
        .filter_map(|instruction| match instruction {
            Instruction::StandardCall { operation, .. } => Some(*operation),
            _ => None,
        })
        .collect::<Vec<_>>();
    for operation in [
    ...
    verify(compilation.module).expect("pure standard bytecode verifies");
}
...
"#;
    ...
    let location = compilation
        ...
        .iter()
        .find(|location| {
            location.function as usize == main_id && location.instruction as usize == internal_get
        })
...
fn strings_and_templates_reject_obsolete_or_invalid_forms_exactly() {
    for (source, code, text) in [
        ...
        (
            "export fn main() returns String { `value ${1}` }",
            "E3011",
            "1",
        ),
    ] {
...
"#;
    let compilation = compile_source(source).expect("direct and transitive declarations compile");
...
#[test]
fn option_coalescing_is_right_associative_lazy_and_exactly_typed() {
    let source = r#"
export fn main() returns (Int, Int, Bool, Int) {
  let a: Option<Int> = None;
  let b: Option<Int> = None;
  (Some(7) ?? (1 / 0), a ?? b ?? 9, Some(false) ?? true || true, Some(11) ?? fail("skipped"))
}
"#;
    let compilation = compile_source(source).expect("coalescing compiles");
    let verified = verify(compilation.module).expect("coalescing bytecode verifies");
    assert_eq!(
        allen_vm::execute(&verified),
        Ok(allen_vm::Value::Tuple(
            vec![
                allen_vm::Value::Int(7),
                allen_vm::Value::Int(9),
                allen_vm::Value::Bool(false),
                allen_vm::Value::Int(11),
            ]
            .into()
        ))
    );
    ...
    for invalid in [
        "export fn main() returns Int { Ok(1) ?? 2 }",
        "export fn main() returns String { Some(1) ?? \"wrong\" }",
    ] {
        let diagnostics = compile_source(invalid).expect_err("invalid coalescing is rejected");
        assert_eq!(diagnostics[0].code, "E3007");
    }
}

#[test]
fn fail_is_pure_never_and_has_a_distinct_terminal_instruction() {
    for reason in ["", "requested failure"] {
        let source = format!(
            "export fn main() returns Int {{ if (true) {{ fail(\"{reason}\") }} else {{ 1 }} }}"
        );
        let compilation = compile_source(&source).expect("fail compiles as Never");
        let main = compilation
            .module
            .functions
            .iter()
            .find(|function| function.name.ends_with("::main"))
            .expect("main bytecode");
        assert!(compilation.module.effect_sets[main.effects as usize].is_empty());
        assert!(compilation.module.functions.iter().any(|function| {
            function
                .code
                .iter()
                .any(|instruction| matches!(instruction, Instruction::Fail { .. }))
        }));
        let verified = verify(compilation.module).expect("fail bytecode verifies");
        assert_eq!(
            allen_vm::execute(&verified),
            Err(allen_vm::VmError::ProgramFailed {
                reason: reason.to_owned()
            })
        );
    }
    ...
    let source = format!("export fn main() returns Never {{ fail(\"{oversized}\") }}");
    let compilation = compile_source(&source).expect("oversized reason compiles");
    assert_eq!(
        allen_vm::execute(&verify(compilation.module).unwrap()),
        Err(allen_vm::VmError::ResourceLimit {
    ...
    for invalid in [
        "export fn main() returns Never { fail() }",
        "export fn main() returns Never { fail(1) }",
        "export fn main() returns Never { fail(\"a\", \"b\") }",
    ] {
        let diagnostics = compile_source(invalid).expect_err("fail contract is exact");
        assert!(matches!(diagnostics[0].code, "E3005" | "E3011"));
    }

    let direct = compile_source("export fn main() returns Never { fail(\"direct\") }")
        .expect("direct fail compiles");
    assert!(direct.mir.functions.iter().any(|function| {
        function
            .blocks
            .iter()
            .any(|block| matches!(block.terminator, MirTerminator::Fail { .. }))
    }));
}

#[test]
fn fail_terminators_survive_nested_if_match_and_coalescing_lowering() {
    for (source, reason) in [
        (
            r#"export fn main() returns Int {
  if (1 == 2) {
    if (true) { fail("nested if") } else { 0 }
  } else {
    1
  }
}"#,
        ...
        (
            r#"export fn main() returns Int {
  match true {
    false => match Some(1) { Some(value) => fail("nested match"), None => 0 },
    true => 2,
  }
}"#,
            None,
  ...
  value ?? fail("missing option")
}"#,
            Some("missing option"),
        ),
    ] {
        let compilation = compile_source(source).expect("nested failure compiles");
        assert!(compilation.mir.functions.iter().any(|function| {
            function
                .blocks
                .iter()
        ...
        let verified = verify(compilation.module).expect("nested failure bytecode verifies");
        match reason {
            ...
            None => assert!(allen_vm::execute(&verified).is_ok()),
        }
    }
}

#[test]
fn newtypes_preserve_nominal_identity_and_require_explicit_access() {
    let source = r#"
newtype EpochSeconds = Int
newtype RetryCount = Int

export fn main() returns (EpochSeconds, Int, Bool, Bool) {
  let epoch = EpochSeconds(41);
  (EpochSeconds(epoch.value + 1), epoch.value, epoch == EpochSeconds(41), epoch < EpochSeconds(42))
}
"#;
    let compilation = compile_source(source).expect("newtypes compile");
    assert!(compilation.module.functions.iter().any(|function| {
        function
            .code
            .iter()
            .any(|instruction| matches!(instruction, Instruction::NewtypeWrap { .. }))
    }));
    assert!(compilation.module.functions.iter().any(|function| {
        function
            .code
            .iter()
            .any(|instruction| matches!(instruction, Instruction::NewtypeUnwrap { .. }))
    }));
    let verified = verify(compilation.module).expect("newtype bytecode verifies");
    let allen_vm::Value::Tuple(values) =
        allen_vm::execute(&verified).expect("newtype program runs")
    else {
        panic!("expected tuple")
    };
    let allen_vm::Value::Newtype(epoch) = &values[0] else {
        panic!("expected wrapped epoch")
    };
    assert!(epoch.identity().ends_with("::EpochSeconds"));
    assert_eq!(epoch.value(), &allen_vm::Value::Int(42));
    assert_eq!(
        &values[1..],
        &[
            allen_vm::Value::Int(41),
            allen_vm::Value::Bool(true),
            allen_vm::Value::Bool(true),
        ]
    );
    ...
    for invalid in [
        "newtype A = Int\nexport fn main() returns A { 1 }",
        "newtype A = Int\nexport fn main() returns A { A(1) + A(2) }",
        "newtype A = Int\nnewtype B = Int\nexport fn main() returns Bool { A(1) == B(1) }",
        "newtype A = Int\nexport fn main() returns Int { let A(value) = A(1); value }",
    ] {
        assert!(compile_source(invalid).is_err(), "must reject {invalid}");
    }
}

#[test]
fn newtypes_import_nominal_identity_and_reject_recursive_or_invalid_underlyings() {
    let sources = BTreeMap::from([
        (
            "main.allen".to_owned(),
            r#"
import { Count as Left } from "./left.allen";
import { Count as Right } from "./right.allen";
export fn main() returns (Left, Right) { (Left(1), Right(1)) }
"#
            .to_owned(),
        ),
        (
            "left.allen".to_owned(),
            "export newtype Count = Int\n".to_owned(),
        ),
        (
            "right.allen".to_owned(),
            "export newtype Count = Int\n".to_owned(),
    ...
    let compilation = compile_bundle("main.allen", &sources).expect("imports compile");
    let identities = compilation
        ...
        .filter_map(|value_type| match value_type {
            ValueType::Newtype { name, .. } => Some(name.as_str()),
            _ => None,
        })
        .collect::<BTreeSet<_>>();
    assert!(
        identities
            .iter()
            .any(|name| name.ends_with("left.allen::Count"))
    );
    assert!(
        identities
            .iter()
            .any(|name| name.ends_with("right.allen::Count"))
    );

    for invalid in [
        "newtype Loop = List<Loop>\nexport fn main() returns Int { 0 }",
        "newtype Callback = fn(Int) returns Int\nexport fn main() returns Int { 0 }",
        "newtype Pending = Future<Int>\nexport fn main() returns Int { 0 }",
        "newtype Nothing = Never\nexport fn main() returns Int { 0 }",
        "newtype Access = ExternalFsAccess\nexport fn main() returns Int { 0 }",
        "newtype Agent = SubAgent\nexport fn main() returns Int { 0 }",
        "newtype Int = String\nexport fn main() returns Int { 0 }",
    ] {
        let diagnostics = compile_source(invalid).expect_err("invalid newtype is rejected");
        assert!(matches!(diagnostics[0].code, "E2012" | "E3011" | "E3005"));
    }
}

#[test]
fn newtypes_support_valid_aggregate_enum_map_key_and_narrow_contracts() {
    let source = r#"
enum Reading { Empty Number(Int) }
newtype Blob = Bytes
newtype Labels = List<String>
newtype ReadingValue = Reading
newtype Index = Int

export fn main() returns (Int, Bool, Bool, Int) {
  let labels = Labels(["a", "b"]);
  let reading = ReadingValue(Reading.Number(3));
  let values: Map<Index, String> = map { Index(2): "later", Index(1): "first" };
  let opaque = to_unknown(Blob(b"x"));
  let recovered = match narrow<Blob>(opaque) {
    Some(value) => value == Blob(b"x")
    None => false
  };
  (length(labels.value), reading == ReadingValue(Reading.Number(3)), recovered, if (values[Index(2)] == "later") { 1 } else { 0 })
}
"#;
    let compilation = compile_source(source).expect("supported newtype shapes compile");
    let output = allen_vm::execute(&verify(compilation.module).unwrap()).unwrap();
    assert_eq!(
        output,
        allen_vm::Value::Tuple(
            vec![
                allen_vm::Value::Int(2),
                allen_vm::Value::Bool(true),
                allen_vm::Value::Bool(true),
                allen_vm::Value::Int(1),
            ]
...
#[test]
fn strict_json_decode_is_a_pure_concrete_generic_builtin() {
    let source = r#"
export fn main() returns Result<(Int, String), DecodeError> {
  decode<(Int, String)>(b"[7,\"ok\"]")
}
"#;
    let compilation = compile_source(source).expect("decode source compiles");
    assert!(
        compilation.module.functions[0]
            .code
            .iter()
            .any(|instruction| {
                matches!(instruction, Instruction::Decode { target: ValueType::Tuple(items), .. }
                if items == &vec![ValueType::Int, ValueType::String])
            })
    );
    for invalid in [
        "export fn main() returns Int { decode<Int>(b\"1\") }",
        "export fn main() returns Result<Int, DecodeError> { decode<Int>(\"1\") }",
        "export fn main() returns Result<Int, DecodeError> { decode<Unknown>(b\"1\") }",
        "export fn main() returns Result<Int, DecodeError> { decode<Int>(b\"1\", b\"2\") }",
    ] {
        assert!(compile_source(invalid).is_err(), "must reject {invalid}");
    }
}
...
#[test]
fn strict_json_decode_preserves_nominal_and_nested_boundary_values() {
    let source = r#"
...
export fn main() returns Int {
  ...
  }>(b"{\"bytes\":{\"$bytes\":\"eA==\"},\"id\":7,\"optional\":{\"tag\":\"Some\",\"value\":\"ok\"},\"state\":{\"tag\":\"Count\",\"value\":[3]},\"values\":[[\"a\",1],[\"b\",2]]}");
  match decoded {
    Ok(value) => value.id.value
    Err(_) => 0
  }
}
"#;
    let compilation = compile_source(source).expect("nested decode source compiles");
    let output = allen_vm::execute(&verify(compilation.module).unwrap()).unwrap();
    assert_eq!(output, allen_vm::Value::Int(7));
}

#[test]
fn package_template_calls_have_exact_closed_record_diagnostics() {
    let module = "pkg://demo@0.1.0/src/main.allen";
    let binding = CompilerTemplateBinding {
        package: "demo@0.1.0".to_owned(),
        name: "notice".to_owned(),
        template: 0,
        holes: vec![
            ("count".to_owned(), ValueType::Int),
            ("title".to_owned(), ValueType::String),
        ],
    };
    let compile = |body: &str| {
        let source = format!("export fn main() returns String {{ {body} }}");
        let bundle = PackageSourceBundle {
            root: module.to_owned(),
            sources: BTreeMap::from([(module.to_owned(), source)]),
            import_targets: BTreeMap::new(),
            entry_points: vec![PackageEntryPoint {
                module: module.to_owned(),
                function: "main".to_owned(),
        ...
        compile_package_bundle_with_prepared_tools_and_templates(
            &bundle,
            &mut prepared,
    ...
    for (body, expected) in [
        (
            "templates.notice.render({ title: \"x\" })",
            "template 'notice' requires hole 'count'",
        ),
        (
            "templates.notice.render({ title: \"x\", count: 1, other: true })",
            "template 'notice' has no hole 'other'",
        ),
        (
            "templates.notice.render({ title: \"x\", count: 1, count: 2 })",
            "duplicate template field 'count'",
        ),
        (
            "templates.notice.render({ title: \"x\", count: \"one\" })",
            "template hole 'count' expects Int, found String",
        ),
        (
        ...
        let diagnostics = compile(body).expect_err(body);
        assert_eq!(diagnostics[0].code, "E3012", "{body}: {diagnostics:?}");
        assert_eq!(diagnostics[0].message, expected, "{body}");
    }

    let loose = compile_source(
        "export fn main() returns String { templates.notice.render({ title: \"x\", count: 1 }) }",
    )
    .expect_err("loose source has no template namespace");
    assert_eq!(loose[0].code, "E3012");
    assert_eq!(
        loose[0].message,
        "template 'notice' is not declared in this package"
    );
}

#[test]
fn record_invariants_compile_as_nominal_bounded_entry_contracts() {
    let source = r#"
record Range { max: Int, min: Int } where { min <= max }
export fn main() returns List<Range> { [Range { min: 1, max: 2 }] }
"#;
    let compilation = compile_source(source).expect("record invariant compiles");
    assert_eq!(compilation.record_invariants.len(), 1);
    let definition = &compilation.record_invariants[0];
    assert_eq!(definition.identity, "main.allen::Range");
    assert_eq!(
        definition
            .fields
            .iter()
    ...
    let main = compilation
        ...
        .find(|function| function.function == "main")
        .unwrap();
    assert!(main.input_validators.is_empty());
    assert_eq!(main.output_validators.len(), 1);
    assert_eq!(
        main.output_validators[0].path,
        vec![allen_bytecode::EntryValidatorPathSegment::ListElement]
    );
}

#[test]
fn record_invariant_sites_preserve_import_identity_across_boundary_containers() {
    let root = "pkg://app@0.1.0/src/main.allen";
    let models = "pkg://models@1.0.0/src/types.allen";
    let bundle = PackageSourceBundle {
        root: root.to_owned(),
        sources: BTreeMap::from([
            (
                root.to_owned(),
                r#"
import { Alias as Imported, Envelope, Same, Wrapped } from "models_pkg/src/types.allen";
export fn main(input: {
  direct: Imported,
  envelope: Envelope,
  false_positive: Same,
  list: List<Imported>,
  mapping: Map<String, Imported>,
...
"#
            ...
            (
                models.to_owned(),
                r#"
export record Checked { high: Int, low: Int } where { low <= high }
export record Same { high: Int, low: Int }
export type Alias = Checked
export newtype Wrapped = Alias
export enum Envelope { One(Alias), Named { value: Wrapped } }
"#
                .to_owned(),
            ),
        ]),
        import_targets: BTreeMap::from([(
            (root.to_owned(), "models_pkg/src/types.allen".to_owned()),
            models.to_owned(),
        )]),
        entry_points: vec![PackageEntryPoint {
            module: root.to_owned(),
        ...
        entry_modules: vec![models.to_owned()],
    };
    let compilation = compile_package_bundle(&bundle).expect("nested imported invariant compiles");
    assert_eq!(compilation.record_invariants.len(), 1);
    assert_eq!(
        compilation.record_invariants[0].identity,
        "pkg://models@1.0.0/src/types.allen::Checked"
    );
    let main = compilation
        .exported_functions
        .iter()
        .find(|function| function.module == root && function.function == "main")
        .unwrap();
    assert_eq!(main.input_validators.len(), 10);
    assert!(main.input_validators.iter().all(|site| {
        !matches!(
            site.path.first(),
            Some(allen_bytecode::EntryValidatorPathSegment::Field(2))
        )
    }));
    assert!(main.output_validators.is_empty());
}

#[test]
fn record_invariants_reject_non_boundary_expression_forms() {
    for (predicate, message) in [
        ...
        let source = format!(
            r#"
record Child {{ value: Int }}
record Checked {{ child: Child, left: Int, right: Int }} where {{ {predicate} }}
fn helper(value: Int) returns Int {{ value }}
export fn main() returns Int {{ 0 }}
"#
        );
        let diagnostics = compile_source(&source).expect_err(predicate);
        assert_eq!(diagnostics[0].code, "E3013", "{predicate}: {diagnostics:?}");
        assert_eq!(diagnostics[0].message, message, "{predicate}");
    }
}
